...
const ProjecteleteId = document.querySelectorAll('.delete-student')

for (let i = 0; i < ProjecteleteId.length; i++) {
  ProjecteleteId[i].addEventListener('click', _ => {
    fetch('/students/delete/'+ProjecteleteId[i].name, {
      method: 'delete',
      headers: { 'Content-Type': 'application/json' },
    })
      .then(res => {
