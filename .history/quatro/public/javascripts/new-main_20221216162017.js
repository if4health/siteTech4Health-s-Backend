...
const PeleteId = document.querySelectorAll('.delete-student')

for (let i = 0; i < PeleteId.length; i++) {
  PeleteId[i].addEventListener('click', _ => {
    fetch('/students/delete/'+PeleteId[i].name, {
      method: 'delete',
      headers: { 'Content-Type': 'application/json' },
    })
      .then(res => {
