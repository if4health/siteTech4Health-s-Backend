...
const eleteId = document.querySelectorAll('.delete-student')

for (let i = 0; i < eleteId.length; i++) {
  eleteId[i].addEventListener('click', _ => {
    fetch('/students/delete/'+eleteId[i].name, {
      method: 'delete',
      headers: { 'Content-Type': 'application/json' },
    })
      .then(res => {
