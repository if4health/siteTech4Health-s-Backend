...
const ProjectDeleteId = document.querySelectorAll('.delete-student')

for (let i = 0; i < ProjectDeleteId.length; i++) {
  ProjectDeleteId[i].addEventListener('click', _ => {
    fetch('/students/delete/'+ProjectDeleteId[i].name, {
      method: 'delete',
      headers: { 'Content-Type': 'application/json' },
    })
      .then(res => {
