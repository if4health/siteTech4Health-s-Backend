...
const ProjectDeleterId = document.querySelectorAll('.delete-student')

for (let i = 0; i < ProjectDeleterId.length; i++) {
  ProjectDeleterId[i].addEventListener('click', _ => {
    fetch('/students/delete/'+ProjectDeleterId[i].name, {
      method: 'delete',
      headers: { 'Content-Type': 'application/json' },
    })
      .then(res => {
...
const updateId = document.querySelectorAll('.update-student')
