...
// const Projec = require('../schema/Projec')
...
mongoose.connect(connString, {dbName : DB_NAME, useNewUrlParser: true, useUnifiedTopology: true})
  .then(client => {
    ...
    // @route PUT /update/:id
    // @desc Upload a Projec status to finish an ongoing Projec work.
    router.put('/update/:id', (req, res) => {
      Projec.findOneAndUpdate(
        { "_id": req.params.id },
        {
          $set: {
        ...
        .then(result => {
          console.log(result)
          res.redirect('/Projecs/');
        })
        .catch(error => console.error(error.message))
    })

    // @route DELETE /delete/:id
    // @desc Delete a Projec by id =) 
    router.delete('/delete/:id', (req, res) => {
      Projec.deleteOne({ "_id": req.params.id })
        .then(result => {
          console.log(result)
          res.json(result)
        })
