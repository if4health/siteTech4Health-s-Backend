...
const { DB_URI, DB_NAME, BASE_PATH } = process.env;
...
mongoose
  ...
  .then(() => {
    ...
    router.get("/", (req, res) => {
      WhiteList.find().then((results) => {
        res.render("whiteList", { whiteList: results });
      });
    });

    router.get("/:email", (req, res) => {
      WhiteList.find({ email: req.params.email }).then((results) => {
    ...
    router.post("/add", (req, res) => {
      ...
      try {
        body.save().then(() => {
          res.status(200).redirect(`${BASE_PATH}/whiteList`);
        });
      } catch (err) {
        console.log(err);
    ...
    router.delete("/delete/:email", (req, res) => {
      try {
        WhiteList.deleteOne({ email: req.params.email }).then(() => {
          res.status(200).redirect(`${BASE_PATH}`);
        });
      } catch (err) {
        console.log(err);
        res.status(504).send("error");
